        ...
        for s in grandi_soggetti:
            if self.curl:
                print "curl -L -o/dev/null -w '%{url_effective} - %{http_code} (%{time_total}sec.)'\n",\
                      "\"http://opencoesione.gov.it/%s{,?tematizzazione=totale_costi,?tematizzazione=totale_pagamenti,?tematizzazione=totale_progetti}\" >> cache_generation.log" % s.get_absolute_url()
            else:
