...
urlpatterns = patterns('',
    ...
    url(r'^temi/(?P<slug>[-\w]+)/regioni.(?P<ext>(json|html))$',
        LeafletView.as_view(inner_filter='temi'), name='territori_leaflet_regioni_tema'),
    url(r'^temi/(?P<slug>[-\w]+)/province.(?P<ext>(json|html))$',
        LeafletView.as_view(inner_filter='temi'), name='territori_leaflet_province_tema'),
    url(r'^nature/(?P<slug>[-\w]+)/regioni.(?P<ext>(json|html))$',
        LeafletView.as_view(inner_filter='nature'), name='territori_leaflet_regioni_natura'),
    url(r'^nature/(?P<slug>[-\w]+)/province.(?P<ext>(json|html))$',
        LeafletView.as_view(inner_filter='nature'), name='territori_leaflet_province_natura'),
    url(r'^programmi/(?P<codice>[\w]+)/regioni.(?P<ext>(json|html))$',
        LeafletView.as_view(inner_filter='programmi'), name='territori_leaflet_regioni_programma'),
    url(r'^programmi/(?P<codice>[\w]+)/province.(?P<ext>(json|html))$',
        LeafletView.as_view(inner_filter='programmi'), name='territori_leaflet_province_programma'),
)