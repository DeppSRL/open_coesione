...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if args:
            ...
            thematization = '?tematizzazione={}'.format(options['thematization']) if options['thematization'] else ''

            try:
                soggetto = Soggetto.objects.get(slug=slug)
            except Soggetto.DoesNotExist:
                raise Exception('Unknown soggetto {}'.format(slug))
        else:
            raise Exception('Please insert a slug for a soggetto')

        self.logger.info(u'Soggetto: {}, Tematizzazione: {}'.format(soggetto.denominazione, thematization))

        url = reverse('soggetti_soggetto', kwargs={'slug': slug})

        if options['clearcache']:
            cache_key = 'context{}{}'.format(url, thematization)
            self.logger.info(u'Clearing the cache for key {}{}'.format(cache_key, thematization))
            cache.delete(cache_key)

        view = setup_view(
            SoggettoView(),
            RequestFactory().get('{}{}'.format(url, thematization)),
            slug=slug,
        )
